                    ...
                    if (typeof fanyijsUrl !== 'undefined') {
                        GM_xmlhttpRequest({
                            ...
                            onload: function (r) {
                                ...
                                var signMatch = /sign:[a-z]{1}\.md5\("fanyideskweb"\+[a-z]{1}\+[a-z]{1}\+"(.*)"\)}};/g.exec(r.responseText.replace(/\s*/g, ''));
                                if (!signMatch) {
    ...
    function GetToken() {
        GM_xmlhttpRequest({
            ...
            onload: function (r) {
                ...
                var gtkMatch = /window\.gtk = "(.*?)"/.exec(html);
                var commonTokenMatch = /token: '(.*?)',/.exec(html);
                if (!gtkMatch) {
                    console.log("获取gtk失败！！！");
                }
                if (!commonTokenMatch) {
